    ...
    public class Rdn
    {
        public static void Init()
        {
            ...
            else
            {
                ...
                Engine = REngine.GetInstance("REngine");// ();
                Engine.Initialize();
            }
        }
            ...
            try
            {
                ...
                switch (sexp.Type)
                {
                    case SymbolicExpressionType.CharacterVector:
                        return PrintDisplay(sexp.AsCharacter().ToArray());//.ToArrayFast());
                    case SymbolicExpressionType.NumericVector:
                        return PrintDisplay(sexp.AsNumeric().ToArray());//.ToArrayFast());
                    case SymbolicExpressionType.IntegerVector:
                        return PrintDisplay(sexp.AsInteger().ToArray());//ToArrayFast());
                    default:
